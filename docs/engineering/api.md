...
Завершает активную тренировку без сохранения в истории и статистике. Тело не
требуется. Endpoint требует cookie-сессию и выполняет удаление только при
одновременном совпадении `id`, текущего `user_id` и `status = 'active'`.
...
Ответ `200` одинаков для удалённой активной, уже удалённой, завершённой и чужой
тренировки. Это делает повтор идемпотентным и не раскрывает существование чужих
или неактивных ID.

Удаление строки `workout_sessions` не вызывает обычный `/complete`, не создаёт
историю и не обновляет агрегаты. Связанные `workout_session_snapshots` и
`workout_sets` удаляются существующими внешними ключами `ON DELETE CASCADE`.
После успеха клиент повторно читает `GET /api/workouts`.
